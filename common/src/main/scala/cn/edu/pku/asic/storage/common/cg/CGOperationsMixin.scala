...
trait CGOperationsMixin {
  ...
  implicit class RDDCGOperations(rdd: SpatialRDD) {
    ...
    def getSpatialPartitioner: SpatialPartitioner = {
      ...
      rdd.partitioner.get.asInstanceOf[SparkSpatialPartitioner].getSpatialPartitioner
    }
    ...
    /**
     * Summarize the RDD and store the summarized data internally. Allows subsequence [[summary]] functions
     * to return the cached result.
     * @return a new RDD that contains the same data but has the summary stored in it.
     */
    def summarize: SpatialRDD = Summary.computeAndCache(rdd)
