...
import cn.edu.pku.asic.earthstorage.common.generator.{DistributionType, RandomSpatialRDD}
import cn.edu.pku.asic.earthstorage.common.geolite.EnvelopeNDLite
import cn.edu.pku.asic.earthstorage.common.io.ReadWriteMixin._
...
trait ReadWriteMixin {
   ...
   * 为SparkContext增加spatialFile、shapeFile、geojsonFile等文件格式读取函数
   ...
   * 为普通SpatialRDD类增加rangeQuery、saveAsShapefile、saveAsGeoJSON等函数
   ...
   * 为PartitionedSpatialRDD类增加rangeQuery函数
