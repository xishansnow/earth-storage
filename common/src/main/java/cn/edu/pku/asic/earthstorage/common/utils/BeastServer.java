...
import cn.edu.pku.asic.earthstorage.common.cli.*;
...
public class BeastServer extends AbstractHandler implements JCLIOperation {
  ...
  protected final Map<Class<? extends cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler>, cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler> webHandlers = new HashMap<>();

  static class WebMethodInfo {
    final Pattern pattern;
    final Class< ? extends cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler> webHandlerClass;
    final Method javaMethod;
    final WebMethod methodAnnotation;

    public WebMethodInfo(Pattern p, Class< ? extends cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler> w, Method m, WebMethod a) {
      this.pattern = p;
  ...
  protected WebMethodInfo[] loadWebMethods() {
    ...
    List<String> handlerClassNames = OperationHelper.readConfigurationXML("earthstorage.xml").get("WebHandlers");
    if (handlerClassNames == null)
      return null;
    for (String handlerClassName : handlerClassNames) {
      try {
        Class<? extends cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler> handlerClass = Class.forName(handlerClassName).asSubclass(cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler.class);
  ...
  public void handle(String target, HttpServletRequest request, HttpServletResponse response, int d) {
    ...
    try {
      ...
      if (i < webMethods.length) {
        Class<? extends cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler> matchingWebHandler = webMethods[i].webHandlerClass;
        Method matchingMethod = webMethods[i].javaMethod;
        // Call the method
        cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler webHandler = webHandlers.get(matchingWebHandler);
        if (webHandler == null) {
      ...
      try {
        ...
        cn.edu.pku.asic.earthstorage.common.utils.AbstractWebHandler.reportError(response, String.format("Error in request '%s' after %f seconds", target, (t2-t1)*1E-9), e);
