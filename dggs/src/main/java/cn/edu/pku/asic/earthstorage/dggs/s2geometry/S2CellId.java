...
import cn.edu.pku.asic.earthstorage.dggs.core.AbstractCellId;
...
public final strictfp class S2CellId extends AbstractCellId<S2CellId> implements Comparable<S2CellId> {
   ...
   */
  public static S2CellId fromPoint(SpherePoint p) {
  ...
  /** Return the leaf cell containing the given SphereLatLng. */
  public static S2CellId fromLatLng(SphereLatLng ll) {
    return fromPoint(ll.toPoint());
  }

  public SpherePoint toPoint() {
    return SpherePoint.normalize(toPointRaw());
  }
  ...
  /** Return the SphereLatLng corresponding to the center of the given cell. */
  public SphereLatLng toLatLng() {
    return new SphereLatLng(toPointRaw());
  }
   ...
   * j-coordinates (see s2geometry.h).
   */
  ...
  /**
   * Convert (face, si, ti) coordinates (see s2geometry.h) to a direction vector (not
   * necessarily unit length).
   */
  private static SpherePoint faceSiTiToXYZ(int face, int si, int ti) {
  ...
  private static S2CellId fromFaceIJWrap(int face, int i, int j) {
    ...
    SpherePoint p = S2Projections.faceUvToXyz(face, s, t);
