...
//import cn.edu.pku.asic.storage.dggs.s2geometry.S2CellId;
//import cn.edu.pku.asic.storage.dggs.s2geometry.S2CellUnion;
 ...
 *
 * Typical usage: {@code S2RegionCoverer coverer; coverer.setMaxCells(5); SphereCap
 * cap = SphereCap.fromAxisAngle(...); S2CellUnion covering;
...
public final strictfp class S2RegionCoverer {
  ...
  // the S2Cell metrics defined in s2geometry.h. For example, to find the cell
   ...
   * region is convex (e.g. an SphereCap or SphereLatLngRect).
   * </ul>
   *
  ...
  public static void getSimpleCovering(
          S2Region region, SpherePoint start, int level, ArrayList<S2CellId> output) {
  ...
  private void getInitialCandidates() {
    ...
    if (maxCells >= 4) {
      ...
      SphereCap cap = region.getCapBound();
