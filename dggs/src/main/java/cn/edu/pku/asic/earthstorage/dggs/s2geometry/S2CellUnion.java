 ...
 * suitable for optimizations based on preparation.
 *
 */
...
//  }
  ...
  public SphereCap getCapBound() {
    ...
    SpherePoint centroid = new SpherePoint(0, 0, 0);
    for (S2CellId id : this) {
      double area = S2Cell.averageArea(id.level());
      centroid = SpherePoint.add(centroid, SpherePoint.mul(id.toPoint(), area));
    }
    if (centroid.equals(new SpherePoint(0, 0, 0))) {
      centroid = new SpherePoint(1, 0, 0);
    } else {
      centroid = SpherePoint.normalize(centroid);
    }
    ...
    SphereCap cap = SphereCap.fromAxisHeight(centroid, 0);
    for (S2CellId id : this) {
  ...
  public SphereLatLngRect getRectBound() {
    SphereLatLngRect bound = SphereLatLngRect.empty();
    for (S2CellId id : this) {
