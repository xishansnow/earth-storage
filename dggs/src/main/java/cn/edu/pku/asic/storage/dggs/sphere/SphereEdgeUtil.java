 ...
 *
 */
public strictfp class SphereEdgeUtil {
  /**
  ...
  public static class EdgeCrosser {
    ...
    private final SpherePoint aCrossB;

    // The fields below are updated for each vertex in the chain.

    // Previous vertex in the vertex chain.
    private SpherePoint c;
     ...
     */
    public EdgeCrosser(SpherePoint a, SpherePoint b, SpherePoint c) {
      this.a = a;
      this.b = b;
      this.aCrossB = SpherePoint.crossProd(a, b);
      restartAt(c);
    }
     ...
     */
    public void restartAt(SpherePoint c) {
      this.c = c;
      acb = -Sphere.robustCCW(a, b, c, aCrossB);
    }
     ...
     * vertex C.
     */
    public int robustCrossing(SpherePoint d) {
      ...
      int bda = Sphere.robustCCW(a, b, d, aCrossB);
      int result;
     ...
     */
    public boolean edgeOrVertexCrossing(SpherePoint d) {
      // We need to copy c since it is clobbered by robustCrossing().
      SpherePoint c2 = new SpherePoint(c.get(0), c.get(1), c.get(2));
     ...
     */
    private int robustCrossingInternal(SpherePoint d) {
      // ACB and BDA have the appropriate orientations, so now we check the
      // triangles CBD and DAC.
      SpherePoint cCrossD = SpherePoint.crossProd(c, d);
      int cbd = -Sphere.robustCCW(c, d, b, cCrossD);
      if (cbd != acb) {
        return -1;
      }

      int dac = Sphere.robustCCW(c, d, a, cCrossD);
      return (dac == acb) ? 1 : -1;
    }
  }
  ...
  public static class RectBounder {
    ...
    private SpherePoint a;

    // The corresponding latitude-longitude.
    private SphereLatLng aLatLng;
     ...
     */
    public void addPoint(SpherePoint b) {
      // assert (S2.isUnitLength(b));

      SphereLatLng bLatLng = new SphereLatLng(b);
      ...
      if (bound.isEmpty()) {
        ...
        bound = bound.union(SphereLatLngRect.fromPointPair(aLatLng, bLatLng));
        ...
        SpherePoint aCrossB = Sphere.robustCrossProd(a, b);
        SpherePoint dir = SpherePoint.crossProd(aCrossB, new SpherePoint(0, 0, 1));
        ...
        if (da * db < 0) {
          ...
          bound = new SphereLatLngRect(lat, bound.lng());
        }
      }
      a = b;
      aLatLng = bLatLng;
     ...
     */
    public SphereLatLngRect getBound() {
      return bound;
    }
  ...
  public static class XYZPruner {
    private SpherePoint lastVertex;
     ...
     */
    public void addEdgeToBounds(SpherePoint from, SpherePoint to) {
      if (!boundSet) {
        boundSet = true;
        xmin = xmax = from.x;
        ymin = ymax = from.y;
    ...
    public void setFirstIntersectPoint(SpherePoint v0) {
     ...
     */
    public boolean intersects(SpherePoint v1) {
      boolean result = true;
  ...
  public static class LongitudePruner {
     ...
     */
    public LongitudePruner(S1Interval interval, SpherePoint v0) {
      this.interval = interval;
      this.lng0 = SphereLatLng.longitude(v0).radians();
    }
     ...
     */
    public boolean intersects(SpherePoint v1) {
      double lng1 = SphereLatLng.longitude(v1).radians();
  ...
  public interface WedgeRelation {
    int test(SpherePoint a0, SpherePoint ab1, SpherePoint a2, SpherePoint b0, SpherePoint b2);
  }
  ...
  public static class WedgeContains implements WedgeRelation {
     ...
     */
    @Override
    public int test(SpherePoint a0, SpherePoint ab1, SpherePoint a2, SpherePoint b0, SpherePoint b2) {
      // For A to contain B (where each loop interior is defined to be its left
      // side), the CCW edge order around ab1 must be a2 b2 b0 a0. We split
      // this test into two parts that test three vertices each.
      return Sphere.orderedCCW(a2, b2, b0, ab1) && Sphere.orderedCCW(b0, a0, a2, ab1) ? 1 : 0;
    }
  }
  ...
  public static class WedgeIntersects implements WedgeRelation {
    ...
    public int test(SpherePoint a0, SpherePoint ab1, SpherePoint a2, SpherePoint b0, SpherePoint b2) {
      ...
      return (Sphere.orderedCCW(a0, b2, b0, ab1) && Sphere.orderedCCW(b0, a2, a0, ab1) ? 0 : -1);
    }
  }
  ...
  public static class WedgeContainsOrIntersects implements WedgeRelation {
    ...
    public int test(SpherePoint a0, SpherePoint ab1, SpherePoint a2, SpherePoint b0, SpherePoint b2) {
      ...
      if (Sphere.orderedCCW(a0, a2, b2, ab1)) {
        // We are in case 1, 5, or 6, or case 2 if a2 == b2.
        return Sphere.orderedCCW(b2, b0, a0, ab1) ? 1 : -1; // Case 1 vs. 2,5,6.
      }
      // We are in cases 2, 3, or 4.
      if (!Sphere.orderedCCW(a2, b0, b2, ab1)) {
        return 0; // Case 3.
      }
  ...
  public static class WedgeContainsOrCrosses implements WedgeRelation {
    ...
    public int test(SpherePoint a0, SpherePoint ab1, SpherePoint a2, SpherePoint b0, SpherePoint b2) {
      ...
      if (Sphere.orderedCCW(a0, a2, b2, ab1)) {
        ...
        if (Sphere.orderedCCW(b2, b0, a0, ab1)) {
      ...
      return Sphere.orderedCCW(a0, b0, a2, ab1) ? 0 : -1; // Case 2,3 vs. 4.
    }
  }
   ...
   */
  public static int robustCrossing(SpherePoint a, SpherePoint b, SpherePoint c, SpherePoint d) {
    ...
    SpherePoint aCrossB = SpherePoint.crossProd(a, b);
    int acb = -Sphere.robustCCW(a, b, c, aCrossB);
    int bda = Sphere.robustCCW(a, b, d, aCrossB);
   ...
   */
  public static boolean vertexCrossing(SpherePoint a, SpherePoint b, SpherePoint c, SpherePoint d) {
    ...
    if (a.equals(d)) {
      return Sphere.orderedCCW(Sphere.ortho(a), c, b, a);
    }
    if (b.equals(c)) {
      return Sphere.orderedCCW(Sphere.ortho(b), d, a, b);
    }
    if (a.equals(c)) {
      return Sphere.orderedCCW(Sphere.ortho(a), d, b, a);
    }
    if (b.equals(d)) {
      return Sphere.orderedCCW(Sphere.ortho(b), c, a, b);
    }
   ...
   */
  public static boolean edgeOrVertexCrossing(SpherePoint a, SpherePoint b, SpherePoint c, SpherePoint d) {
  ...
  static class CloserResult {
    ...
    private SpherePoint vmin;

    public double getDmin2() {
      return dmin2;
    }

    public SpherePoint getVmin() {
      return vmin;
    }

    public CloserResult(double dmin2, SpherePoint vmin) {
      this.dmin2 = dmin2;
      this.vmin = vmin;
    }

    public void replaceIfCloser(SpherePoint x, SpherePoint y) {
      // If the squared distance from x to y is less than dmin2, then replace
      // vmin by y and update dmin2 accordingly.
      double d2 = SpherePoint.minus(x, y).norm2();
   ...
   */
  public static SpherePoint getIntersection(SpherePoint a0, SpherePoint a1, SpherePoint b0, SpherePoint b1) {
    ...
    SpherePoint aNorm = SpherePoint.normalize(Sphere.robustCrossProd(a0, a1));
    SpherePoint bNorm = SpherePoint.normalize(Sphere.robustCrossProd(b0, b1));
    SpherePoint x = SpherePoint.normalize(Sphere.robustCrossProd(aNorm, bNorm));
    ...
    if (x.dotProd(SpherePoint.add(SpherePoint.add(a0, a1), SpherePoint.add(b0, b1))) < 0) {
      x = SpherePoint.neg(x);
    }
    ...
    if (Sphere.orderedCCW(a0, x, a1, aNorm) && Sphere.orderedCCW(b0, x, b1, bNorm)) {
      return x;
    }
    ...
    if (Sphere.orderedCCW(b0, a0, b1, bNorm)) {
      r.replaceIfCloser(x, a0);
    }
    if (Sphere.orderedCCW(b0, a1, b1, bNorm)) {
      r.replaceIfCloser(x, a1);
    }
    if (Sphere.orderedCCW(a0, b0, a1, aNorm)) {
      r.replaceIfCloser(x, b0);
    }
    if (Sphere.orderedCCW(a0, b1, a1, aNorm)) {
   ...
   */
  public static double getDistanceFraction(SpherePoint x, SpherePoint a0, SpherePoint a1) {
   ...
   */
  public static S1Angle getDistance(SpherePoint x, SpherePoint a, SpherePoint b) {
    return getDistance(x, a, b, Sphere.robustCrossProd(a, b));
  }
   ...
   */
  public static S1Angle getDistance(SpherePoint x, SpherePoint a, SpherePoint b, SpherePoint aCrossB) {
    Preconditions.checkArgument(Sphere.isUnitLength(x));
    Preconditions.checkArgument(Sphere.isUnitLength(a));
    Preconditions.checkArgument(Sphere.isUnitLength(b));
    ...
    if (Sphere.simpleCCW(aCrossB, a, x) && Sphere.simpleCCW(x, b, aCrossB)) {
    ...
    double linearDist2 = Math.min(SpherePoint.minus(x, a).norm2(), SpherePoint.minus(x, b).norm2());
   ...
   *
   */
  public static SpherePoint getClosestPoint(SpherePoint x, SpherePoint a, SpherePoint b) {
    Preconditions.checkArgument(Sphere.isUnitLength(x));
    Preconditions.checkArgument(Sphere.isUnitLength(a));
    Preconditions.checkArgument(Sphere.isUnitLength(b));

    SpherePoint crossProd = Sphere.robustCrossProd(a, b);
    // Find the closest point to X along the great circle through AB.
    SpherePoint p = SpherePoint.minus(x, SpherePoint.mul(crossProd, x.dotProd(crossProd) / crossProd.norm2()));

    // If p is on the edge AB, then it's the closest point.
    if (Sphere.simpleCCW(crossProd, a, p) && Sphere.simpleCCW(p, b, crossProd)) {
      return SpherePoint.normalize(p);
    }
    // Otherwise, the closest point is either A or B.
    return SpherePoint.minus(x, a).norm2() <= SpherePoint.minus(x, b).norm2() ? a : b;
  }

  /** Constructor is private so that this class is never instantiated. */
  private SphereEdgeUtil() {
  }
}
