...
import cn.edu.pku.asic.earthstorage.dggs.sphere.SpherePoint;
...
public abstract strictfp class S2EdgeIndex {
  ...
  public final void computeIndex() {
    ...
    for (int i = 0; i < getNumEdges(); ++i) {
      SpherePoint from = edgeFrom(i);
      SpherePoint to = edgeTo(i);
  ...
  protected abstract SpherePoint edgeFrom(int index);

  protected abstract SpherePoint edgeTo(int index);
   ...
   */
  protected void findCandidateCrossings(SpherePoint a, SpherePoint b, List<Integer> candidateCrossings) {
   ...
   */
  private static S2CellId containingCell(SpherePoint pa, SpherePoint pb, SpherePoint pc, SpherePoint pd) {
   ...
   */
  private static S2CellId containingCell(SpherePoint pa, SpherePoint pb) {
    S2CellId a = S2CellId.fromPoint(pa);
    S2CellId b = S2CellId.fromPoint(pb);
  ...
  private int getCovering(
      SpherePoint a, SpherePoint b, boolean thickenEdge, ArrayList<S2CellId> edgeCovering) {
    edgeCovering.clear();
    ...
    if (!thickenEdge) {
      ...
      if (idealLevel == S2CellId.MAX_LEVEL) {
        ...
        SpherePoint pq = SpherePoint.mul(SpherePoint.minus(b, a), THICKENING);
        SpherePoint ortho =
            SpherePoint.mul(SpherePoint.normalize(SpherePoint.crossProd(pq, a)), edgeLength * THICKENING);
        SpherePoint p = SpherePoint.minus(a, pq);
        SpherePoint q = SpherePoint.add(b, pq);
        ...
        // Theta(THICKENING).
        containingCellId =
            containingCell(SpherePoint.minus(p, ortho), SpherePoint.add(p, ortho), SpherePoint.minus(q, ortho),
                SpherePoint.add(q, ortho));
      }
    }
    ...
    // cap center.
    SpherePoint middle = SpherePoint.normalize(SpherePoint.div(SpherePoint.add(a, b), 2));
   ...
   */
  private static boolean lenientCrossing(SpherePoint a, SpherePoint b, SpherePoint c, SpherePoint d) {
    // assert (S2.isUnitLength(a));
    // assert (S2.isUnitLength(b));
    // assert (S2.isUnitLength(c));

    double acb = SpherePoint.crossProd(a, c).dotProd(b);
    double bda = SpherePoint.crossProd(b, d).dotProd(a);
    ...
    double cbd = SpherePoint.crossProd(c, b).dotProd(d);
    double dac = SpherePoint.crossProd(c, a).dotProd(c);
   ...
   */
  private static boolean edgeIntersectsCellBoundary(SpherePoint a, SpherePoint b, S2Cell cell) {
    SpherePoint[] vertices = new SpherePoint[4];
    for (int i = 0; i < 4; ++i) {
      vertices[i] = cell.getVertex(i);
    }
    for (int i = 0; i < 4; ++i) {
      SpherePoint fromPoint = vertices[i];
      SpherePoint toPoint = vertices[(i + 1) % 4];
   ...
   * intersect with edge.
   */
  private void getEdgesInChildrenCells(SpherePoint a, SpherePoint b, List<S2CellId> cover,
  ...
  public static class DataEdgeIterator {
     ...
     * cross the edge (a,b).
     */
    public void getCandidates(SpherePoint a, SpherePoint b) {
