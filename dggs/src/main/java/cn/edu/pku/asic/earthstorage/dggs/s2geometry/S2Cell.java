...
import cn.edu.pku.asic.earthstorage.dggs.sphere.*;
//import S2Projections;
...
public final strictfp class S2Cell extends AbstractCell<S2CellId,S2Cell> implements S2Region {
  ...
  // Convenience methods.
  public S2Cell(SpherePoint p) {
    init(S2CellId.fromPoint(p));
  }

  public S2Cell(SphereLatLng ll) {
  ...
  public SpherePoint getVertex(int k) {
    return SpherePoint.normalize(getVertexRaw(k));
  }
   ...
   * @return
   */
  public SpherePoint getVertexRaw(int k) {
    // Vertices are returned in the order SW, SE, NE, NW.
    return S2Projections.faceUvToXyz(face, uv[0][(k >> 1) ^ (k & 1)], uv[1][k >> 1]);
  }

  public SpherePoint getEdge(int k) {
    return SpherePoint.normalize(getEdgeRaw(k));
  }

  public SpherePoint getEdgeRaw(int k) {
    switch (k) {
      case 0:
      ...
      case 2:
        return SpherePoint.neg(S2Projections.getVNorm(face, uv[1][1])); // North
      default:
        return SpherePoint.neg(S2Projections.getUNorm(face, uv[0][0])); // West
    }
  }
   ...
   * length.
   */
  public SpherePoint getCenter() {
    return SpherePoint.normalize(getCenterRaw());
  }

  public SpherePoint getCenterRaw() {
  ...
  public double approxArea() {
    ...
    double flatArea = 0.5 * SpherePoint.crossProd(
        SpherePoint.sub(getVertex(2), getVertex(0)), SpherePoint.sub(getVertex(3), getVertex(1))).norm();
  ...
  public double exactArea() {
    SpherePoint v0 = getVertex(0);
    SpherePoint v1 = getVertex(1);
    SpherePoint v2 = getVertex(2);
    SpherePoint v3 = getVertex(3);
  ...
  public SphereCap getCapBound() {
    ...
    SphereCap cap = SphereCap.fromAxisHeight(SpherePoint.normalize(S2Projections.faceUvToXyz(face, u, v)), 0);
    for (int k = 0; k < 4; ++k) {
  ...
  public SphereLatLngRect getRectBound() {
    if (level > 0) {
      ...
      return new SphereLatLngRect(lat, lng.expanded(MAX_ERROR));
    }
  ...
  private double getLatitude(int i, int j) {
    SpherePoint p = S2Projections.faceUvToXyz(face, uv[0][i], uv[1][j]);
